...
def convert_to_openai_responses_messages(
    ...
    for m in messages:
        ...
        if mtype == "code":
            # Optional: include as assistant-visible context (fenced). Tool calling is handled elsewhere.
            code = m.get("content", "")
            lang = m.get("format", "") or ""
            fenced = f"```{lang}\n{code}\n```"
            out.append({"role": "assistant", "content": _parts_for_text("assistant", fenced)})
            continue
